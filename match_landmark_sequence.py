...
if __name__ == "__main__":
    ...
    for id, before in query_dict.items():
        print(f"{id}, {before}")
        current_landmark = LandmarkDataService.get_landmark_by_id(id, locgi_url)
        current_alias = current_landmark.get("alias", [])
        filtered_alias_by_landmark[id] = []
        for alias in current_alias:
            if alias not in before:
                filtered_alias_by_landmark[id].append(alias)
